...
--run battle simulation on given entity list
function Warfare:runSimulation(entList)
    self.Simulation.entList = entList
    self.Simulation.preset = self:createPreset(self.Simulation.entList)

    self:despawnAll(self.Simulation.entList)
    self:runIteration()
end
...
function Warfare:stopSimulation()
    ...
    self:despawnAll(self.Simulation.entList)
    self:spawnFromPreset(self.Simulation.preset, self.Simulation.entList)
...
function Warfare:getWinner(entList)
    local numAlive = {0, 0}
